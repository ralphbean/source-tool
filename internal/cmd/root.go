...
func CheckAuthWithHostname(hostname string) (*auth.Authenticator, error) {
	...
	if user == nil {
		...
		fmt.Println()
		fmt.Println("Please log in before using sourcetool.")
		fmt.Println()
		fmt.Println("For GitHub, run:")
		fmt.Println("  sourcetool auth login")
		fmt.Println()
		fmt.Println("For GitLab, set the GITLAB_TOKEN environment variable:")
		fmt.Println("  export GITLAB_TOKEN=your-token-here")
		fmt.Println()
